	...
	function (req, res) {
		...
		if (!skiResort) {
			...
			snowHelper.getSnowData(trasformedSkiResort).then(function (snowData) {
				const results = snowHelper.formatSnowResults(snowData, skiResort)
				// send the card with the resort name as the title and snow report data
				res.card(snowHelper.capitalizeFirstLetterResortTitle(skiResort), results);
				// send the user the requested snow report
				res.say(results).send();
			}).catch(function (err) {
